...
We need to find a service that start as ```SYSTEM```. We can do it by using the ```sc qc SERVICE_NAME``` command or the [](https://docs.microsoft.com/en-us/sysinternals/downloads/psservice) as follows :
...
In Powershell 5.0 and higer, its history is stored in this file : ```C:\Users\username>\AppData\Roaming\Microsoft\Windows\PowerShell\PSReadLine\ConsoleHost_history.txt```
